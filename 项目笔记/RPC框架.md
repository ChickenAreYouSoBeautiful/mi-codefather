...
## 为什么需要RPC?
	回顾概念：RPC允许一个程序（服务消费方）像调用自己程序的方法一样调用远程方法，调用另一个程序（服务提供方）的接口，而且并不需要了解他底层的通讯和数据的传输细节等。这些都会由RPC框架帮你完成，使得开发者可以更轻松的完成调用远程服务。

	他可以像调用本地方法一样调用远程方法。当我们有两个系统分别为A和B，如果A系统需要用到B系统中的功能，首先A和B服务都是独立的，所以不能通过引入SDK来解决。这种情况B作为服务的提供方，需要暴露接口。服务的消费方A通过构建HTTP请求来进行访问。如果B服务需要调用更多的服务接口，每个服务方法都写一次HTTP请求，那么就会很麻烦。
...
注册中心： 提供者，将提供的接口注册。消费者从注册中心获取。

代理对象： 提供者调用消费者的代理对象。

请求客户端： 发送请求。

![画板](https://cdn.nlark.com/yuque/0/2024/jpeg/32636653/1716275435941-ecfe0d4d-74d1-4a9e-89e9-c5056b0674a4.jpeg)

这就是一个简易版的RPC框架需要的模块。

## 创建模块
common - 用于存储实体类的Service接口

production -服务提供者，接口的提供者

consumer -服务消费者，调用其他服务。

rpcServer-easy -RPC框架简易版。

**common**

![](https://cdn.nlark.com/yuque/0/2024/png/32636653/1716275878385-f1228ec8-2628-4565-a82e-5e360e7092a3.png)

**production**

** **![](https://cdn.nlark.com/yuque/0/2024/png/32636653/1716276044194-228a1ccb-fe8c-4ee6-afe7-9e6287aea4c0.png)

**consumer**

![](https://cdn.nlark.com/yuque/0/2024/png/32636653/1716276160753-1bf4052e-6036-4217-a1c7-cb426e51d8ad.png)
...
server  - web服务及vertx的web服务实现

![](https://cdn.nlark.com/yuque/0/2024/png/32636653/1716276205482-c210ece8-235b-4ffc-8288-9ceb2f75faef.png)

运行梳理流程（可以配合debug更好理解）：

提供者开启服务配置handler，注册提供的服务

消费者消费，通过proxy工厂获取指定类的代理对象 。

代理对象构建参数序列化后通过HTTP请求访问提供者。

Vertx启动时配置的handler接收请求。

反序列化参数，根据参数通过反射调用方法，序列化后再返回响应结果。
...
需求：
 ...
 我们需要Rpc框架可以读取引入RPC的模块的配置文件加载全局配置，用于替换硬编码。
...
Rpc框架的核心功能是调用其他远程服务器，但我们在实际开发和测试过程中，有时无法直接访问真实的远程服务或者访问真实的远程服务会产生不可控的影响，比如服务不稳定等。

mock是指模拟对象，通常用于测试，便于我们跑通业务流程。

为什么支持mock?

虽然mock不是Rpc框架的核心能力，但他的开发成本并不高，而且给RPC框架支持mock后，开发者可以更轻松的调用接口，不依赖真实的远程服务。提高使用体验。
...
#### 主流的序列化器
主流的序列化器有JSON,Hessian,Kryo,protobuf等。
...
### 自定义序列化器
当开发者不想使用我们现有的序列化器，该如何做呢？

思路：Rpc框架只要能读取到用户自定义序列化器的位置时，就可以加载这个类，作为Serializable接口。
...
SPI机制可以通过读取特定的配置文件将自己的实现注册到系统中，然后系统通过反射机制动态的加载实现。而不需要修改原有的框架代码，就可以实现系统的解耦极大的增加了扩展性。
...
系统实现SPI虽然简单，但如果我们想定制多个不同的接口实习，就没办法在框架中指定使用哪一个。
...
## 开发实现
### 1) 多种序列化器的实现
实现了josn,Hessian,Kryo主流的序列化方式，建议参考代码。
...
# 基本注册中心实现
## 需求分析
rpc框架的其中一个核心模块就是注册中心，目的是帮助消费者获取提供者的调用地址，从而达到不许要硬编码到项目中。

在之前我们使用concurrentHashMap来作为本地缓存。

![画板](https://cdn.nlark.com/yuque/0/2024/jpeg/32636653/1717116005784-e865dee7-88b2-4613-9e77-8e34fffaa1a9.jpeg)
...
通过技术选型确定使用etcd作为默认的存储服务，原因：他是go语言实现的性能更高，他支持事务，并且底层使用了Raft算法，保证了他的数据一致性。

定义registerConfig，用户存储注册中心的信息，如注册中心名称，地址，端口等。

定义serverMetaInfo，用于存储服务的信息，如服务名，地址，端口，版本等。

定义Register接口，和EtcdRegister实现类，实现服务注册，服务发现，服务注销等操作
...
为什么需要？
 	...
 	用更少的空间来传递需要的信息。目前使用的是http协议，他是应用层协议，他的请求头和信息较多空间占的大，并且请求和响应是都会经过七层封装，都会影响数据传输的速度。所以我们可以考虑使用TCP，他是底层传输层的协议性能是要比应用层的高些，并可以有更多的自主设计空间，
...

{
  	...
  	魔数： 安全校验，防止服务器处理非框架发出请求。

版本号： 控制版本，保持一致性

序列化器： 告诉客户端和服务端如何解析数据
...
![画板](https://cdn.nlark.com/yuque/0/2024/jpeg/32636653/1721292731082-11fe0099-2d1d-4a9d-ab04-cb97ee7bcbb3.jpeg)

## 粘包/半包
粘包,就是发送方发送的多个数据包被接收方合并为一个数据包，导致接收方无法正确的区分每个数据包的边界。

半包，就算发送方发送的数据包较大被分成多个片段传输或网络波动导致接收方接收的数据并不完整。

解决： 半包我们可以定义一个长度，如长度不满足的化就先不读等下一次再读。粘包是只读取指定长度的数据，过长的数据我们留到下一次再读。

现在我们的header长度可以固定，但是消息体长度是不固定的。那么我们可以分成两次读取，第一次先读取固定的header,第二次根据header中存储的bodyLength读取Body,最后重置一下。就能读取到一次完成的数据了
...
## 负载均衡
当我们系统请求过多时，单台机器无法承受压力。就会去部署多个服务提供者来提供服务，缓解压力。那我们又如何保证请求去按照我们的想法去请求不同的服务提供者呢？

常见的有nginx(七层协议负载均衡)

### 常见的负载均衡算法
负载均衡就是按照我们的想法，将请求转发到不同的服务器上。按照什么策略选择什么资源，所以主要学习的是他的算法。

轮询 : 循环选择服务

加权轮询： 循环选择服务，并根据权重增加循环几率

随机 ： 随机选择服务

加权随机 : 随机选择服务,并根据权重增加随机几率

一致性hash ：维护一个hash环，将服务作为一个节点放到这个环里，通过计算hash值来取hash环中大于或等于hash环中的值。适用于长连接的情况。
...
2.指数退避重试：比如第一次重试为1秒，第二次为3秒（增加两秒），第四次为7秒（增减四秒），以此类推

3.随机延迟重试：在每次重试前适用随机的时间间隔，避免请求的同时发生

4.可变延迟重试：根据前重试的情况来动态调整下一次重试的时间间隔

因为没有特殊需求，我们配置了固定间隔重试。

#### 停止重试
停止重试的策略常见的有

1.最大重试次数： 重试测试达到配置的最大重试次数

2.超时停止：请求超过配置的超时时间，停止重试

由于我们目前的场景是rpc客户端调用，服务提供者的接口，接口的情况我们是不清楚的，所以我们先择最大重试次数作为停止重试的策略
...
![画板](https://cdn.nlark.com/yuque/0/2024/jpeg/32636653/1721878416454-32fd9a95-1ed2-48c4-8a1a-97ba21e05679.jpeg)
...
## 踩坑日志
莫名奇妙的错误，debug找了半天发现是因为端口冲突了，因为我的etcd可视化占的是8080端口，我提供者服务配置的也是8080端口，导致消费者请求到etcd可视化页面上，响应后解析报错

![](https://cdn.nlark.com/yuque/0/2024/png/32636653/1719816186241-f2c9d616-e78d-4614-9dcd-2dc063d1ae4f.png)
...
## Rpc项目总结
   这个项目我还是做了挺久，这是一个纯文本的手写Rpc框架的教程，相比于视频我感觉纯文本更能迫使我去思考。

学到的内容:

1. Rpc的概念，和Rpc的基础功能和大致流程。
2. 多种序列化方式
3. Java的spi机制通过配置类的全限定名在运行时获取类的是实现
4. vertx，利用tcp来实现通讯。
5. 半包/粘包问题和解决
6. etcd分布式存储，k-v结构，实现注册中心用于存储生产者信息。

目前也就扩展了多种配置文件的读取和异常处理。

写完这个项目回过头来看，还是会发现自己思考的不足，根本想不到要这么做。也算是自己经验的不足。如通过SPI机制做的一个个扩展等等。但也学到了一个道理，当遇到自己不会做的功能时，去网上找找别人是否有做过类似的。借鉴学习一下，就会对这个功能如何实现比之前清晰的多。

项目地址

[ChickenAreYouSoBeautiful/RpcServer: 一个简单的Rpc框架（学习版) (github.com)](https://github.com/ChickenAreYouSoBeautiful/RpcServer)
